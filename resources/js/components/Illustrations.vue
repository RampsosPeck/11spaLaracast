<template>
    <div>
        <h1 class="font-normal text-3xl text-grey-darkest leading-none mb-10">
            Illustrations
        </h1>

        <div class="flex mb-12">
            <picture
                class="rounded-lg bg-blue py-6 px-10 mr-10 flex-1 flex justify-center items-center"
            >
                <img
                    src="/img/illustrations/clarity.svg"
                    alt="Clarity Illustration"
                    class="w-32"
                />
            </picture>

            <div style="flex: 1.4">
                <h2 class="font-light text-xl mb-4 mt-4">Clarity</h2>

                <p
                    class="leading-relaxed text-grey-darker text-sm mb-8 font-light"
                >
                    Learning isn’t easy. Keep your mind clear and don’t fill it
                    with expectations. Sponge brain is the best brain.
                </p>

                <a href="/img/illustrations/clarity.svg" class="button">
                    clarity.svg
                </a>
            </div>
        </div>

        <div class="flex mb-12">
            <picture
                class="rounded-lg bg-blue py-6 px-10 mr-10 flex flex-1 justify-center items-center"
            >
                <img
                    src="/img/illustrations/fun.svg"
                    alt="Fun Illustration"
                    class="w-32"
                />
            </picture>

            <div style="flex: 1.4">
                <h2 class="font-light text-xl mb-4 mt-4">Fun</h2>

                <p
                    class="leading-relaxed text-grey-darker text-sm mb-8 font-light"
                >
                    If you’re not having fun, you’re doing something wrong. You
                    know what they say, love what you do and you’ll never work a
                    day in your life.
                </p>

                <a href="/img/illustrations/fun.svg" class="button">
                    fun.svg
                </a>
            </div>
        </div>

        <div class="flex mb-12">
            <picture
                class="rounded-lg bg-blue py-6 px-10 mr-10 flex flex-1 justify-center items-center"
            >
                <img
                    src="/img/illustrations/experiment.svg"
                    alt="Experiment Illustration"
                    class="w-32"
                />
            </picture>

            <div style="flex: 1.4">
                <h2 class="font-light text-xl mb-4 mt-4">Experiment</h2>

                <p
                    class="leading-relaxed text-grey-darker text-sm mb-8 font-light"
                >
                    If you’re not having experiment, you’re doing something
                    wrong. You know what they say, love what you do and you’ll
                    never work a day in your life.
                </p>

                <a href="/img/illustrations/experiment.svg" class="button">
                    experiment.svg
                </a>
            </div>
        </div>

        <div class="flex mb-12">
            <picture
                class="rounded-lg bg-blue py-6 px-10 mr-10 flex flex-1 justify-center items-center"
            >
                <img
                    src="/img/illustrations/strength.svg"
                    alt="Strength Illustration"
                    class="w-32"
                />
            </picture>

            <div style="flex: 1.4">
                <h2 class="font-light text-xl mb-4 mt-4">Strength</h2>

                <p
                    class="leading-relaxed text-grey-darker text-sm mb-8 font-light"
                >
                    If you’re not having strength, you’re doing something wrong.
                    You know what they say, love what you do and you’ll never
                    work a day in your life.
                </p>

                <a href="/img/illustrations/strength.svg" class="button">
                    strength.svg
                </a>
            </div>
        </div>

        <div class="flex mb-12">
            <picture
                class="rounded-lg bg-blue py-6 px-10 mr-10 flex flex-1 justify-center items-center"
            >
                <img
                    src="/img/illustrations/information.svg"
                    alt="Information Illustration"
                    class="w-32"
                />
            </picture>

            <div style="flex: 1.4">
                <h2 class="font-light text-xl mb-4 mt-4">Information</h2>

                <p
                    class="leading-relaxed text-grey-darker text-sm mb-8 font-light"
                >
                    If you’re not having information, you’re doing something
                    wrong. You know what they say, love what you do and you’ll
                    never work a day in your life.
                </p>

                <a href="/img/illustrations/information.svg" class="button">
                    information.svg
                </a>
            </div>
        </div>
    </div>
</template>

<script>
export default {};
</script>