<template>
    <div>
        <h1 class="font-normal text-3xl text-grey-darkest leading-none mb-12">
            Meet "Lary"
        </h1>

        <div class="flex mb-12">
            <div class="mr-8 flex flex-col items-center">
                <img
                    src="/img/mascot/larydefault.svg"
                    alt="Laracasts Default Mascot"
                    class="mb-4"
                />

                <a href="/img/mascot/larydefault.svg" class="button">
                    larydefault.svg
                </a>
            </div>

            <p class="leading-relaxed w-64 mt-8">
                Quirky. Dynamic. Smartsy. He’s got it all. Who wouldn’t want
                him?
            </p>
        </div>

        <div class="flex">
            <!-- larypeek.svg -->
            <div
                style="width: 153px; height: 219px;"
                class="border border-grey-lighter rounded-lg flex flex-col items-center justify-center pt-6 pb-6 px-8 mr-6"
            >
                <a href="/img/mascot/larypeek.svg" class="mb-5 mt-auto">
                    <img
                        src="/img/mascot/larypeek.svg"
                        alt="Laracasts Peeking Mascot"
                    />
                </a>

                <a
                    href="/img/mascot/larypeek.svg"
                    class="text-grey text-2xs hover:underline mt-auto"
                    >larypeek.svg</a
                >
            </div>

            <!-- larywink -->
            <div
                style="width: 153px; height: 219px;"
                class="border border-grey-lighter rounded-lg flex flex-col items-center justify-center pt-6 pb-6 px-8 mr-6"
            >
                <a href="/img/mascot/larywink.svg" class="mb-5 mt-auto">
                    <img
                        src="/img/mascot/larywink.svg"
                        alt="Laracasts Winking Mascot"
                    />
                </a>

                <a
                    href="/img/mascot/larywink.svg"
                    class="text-grey text-2xs hover:underline mt-auto"
                    >larywink.svg</a
                >
            </div>

            <!-- larysmart -->
            <div
                style="width: 153px; height: 219px;"
                class="border border-grey-lighter rounded-lg flex flex-col items-center justify-center pt-6 pb-6 px-8 mr-6"
            >
                <a href="/img/mascot/larysmart.svg" class="mb-5 mt-auto">
                    <img
                        src="/img/mascot/larysmart.svg"
                        alt="Laracasts Smart Mascot"
                    />
                </a>

                <a
                    href="/img/mascot/larysmart.svg"
                    class="text-grey text-2xs hover:underline mt-auto"
                    >larysmart.svg</a
                >
            </div>

            <!-- larylab -->
            <div
                style="width: 153px; height: 219px;"
                class="border border-grey-lighter rounded-lg flex flex-col items-center justify-center pt-6 pb-6 px-8"
            >
                <a href="/img/mascot/larylab.svg" class="mb-5 mt-auto">
                    <img
                        src="/img/mascot/larylab.svg"
                        alt="Laracasts Experimenting Mascot"
                        style="width: 74px"
                    />
                </a>

                <a
                    href="/img/mascot/larylab.svg"
                    class="text-grey text-2xs hover:underline mt-auto"
                    >larylab.svg</a
                >
            </div>
        </div>
    </div>
</template>

<script>
export default {};
</script>